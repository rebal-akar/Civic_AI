"""SemEval-2020 Task 11 evaluation metrics (Da San Martino et al., 2020).

SI (Span Identification) — technique-agnostic character-overlap F1:
    P(S,T) = (1/|S|) * Σ_{s∈S,t∈T} |s ∩ t| / |t|
    R(S,T) = (1/|T|) * Σ_{s∈S,t∈T} |s ∩ t| / |s|
    F1 = harmonic mean of P and R

Overlapping predicted spans are merged before scoring. The joint SI+TC metric
uses the same formulas but only counts overlap when technique labels match.
"""
...
from src.schemas import Article, Prediction, Technique
...
class SpanMetrics:
    ...
    f1_delta: float = 0.0  # si_f1 - tc_f1
...
def evaluate(
    ...
    """Compute SemEval-2020 Task 11 SI and joint SI+TC metrics."""
    si_p_num = si_r_num = tc_p_num = tc_r_num = 0.0
    si_p_den = si_r_den = tc_p_den = tc_r_den = 0
    ...
    total_gold = total_pred_raw = total_pred_merged = total_unresolved = 0
    ...
    for article in articles:
        ...
        # SI: merge overlapping predictions before scoring (per SemEval paper).
        merged_intervals = _merge_overlapping_spans(
            [(s.start, s.end) for s in raw_pred_spans]
        )
        total_pred_merged += len(merged_intervals)

        # SI Precision: for each merged predicted span, max over gold of |s∩t|/|t|.
        ...
        # SI Recall: for each gold span, max over merged predicted of |s∩t|/|s|.
        si_r_den += len(gold_spans)
        for gs in gold_spans:
            best = 0.0
            for (ms, me) in merged_intervals:
        ...
        # Joint SI+TC: same formula direction but technique must match, no merging.
        ...
        for gs in gold_spans:
            ...
            # Capture technique-classification confusions for error analysis.
            if best_tc == 0.0:
                for ps in raw_pred_spans:
                    if ps.technique != gs.technique:
                        overlap = _char_overlap(gs.start, gs.end, ps.start, ps.end)
...
def _merge_overlapping_spans(spans: list[tuple[int, int]]) -> list[tuple[int, int]]:
    """Merge overlapping character spans per SemEval: 'all overlapping annotations,
    independently of their techniques, are merged first'.
    """
    if not spans:
        return []
...
def print_results(metrics: SpanMetrics, strategy: str = "") -> None:
    ...
    print(f"  Gold: {metrics.total_gold_spans}  |  "
          f"Pred: {metrics.total_pred_spans_raw} (merged: {metrics.total_pred_spans})  |  "
...
def print_comparison_table(
    ...
    parts = [
        f"{label}={strat}({val:.4f})"
        for label, (strat, val) in best_row.items() if val > 0
    ]
    print(f"\n  Best: {', '.join(parts)}")
    print(f"{'='*w}\n")
...
def print_technique_comparison(
    ...
    for s in strategies:
        all_techniques.update(
            results_by_strategy[s].get("metrics", {}).get("per_technique", {}).keys()
        )
