"""Run all strategies and produce a comparison table and charts.

Usage:
    python -m src.run_all --model gpt-4o --max-articles 10
    python -m src.run_all --model gpt-4o --strategies consol,hybrid --max-articles 50
"""
import argparse
import asyncio
import json
...
from src.evaluation.metrics import print_comparison_table, print_technique_comparison
...
async def run_all(args):
    ...
    for strategy in selected:
        print(f"\n{'='*60}\n  Running: {strategy} ({args.model})\n{'='*60}\n")

        cfg_name = f"{strategy}_{args.model}"
        if args.max_articles is not None:
            cfg_name += f"_n{args.max_articles}"
    ...
    print_comparison_table(all_results, model=args.model, num_articles=len(articles))
    ...
    output_path = (
        output_dir
        / f"comparison_{args.model}_{data_tag}_{strategies_slug}_{limit_slug}_{args.eval_mode}.json"
    )
    ...
    chart_paths = generate_comparison_charts(all_results, output_path.with_suffix(""))
    if chart_paths:
